...
DROP PROCEDURE IF EXISTS `melisr`.`replace_higher_geography_record` $$
CREATE PROCEDURE `melisr`.`replace_higher_geography_record` (in_geography_id INTEGER(11), in_node_number INT, in_geography_rank VARCHAR(16))
BEGIN
    ...
    DECLARE higher_geography_cursor CURSOR FOR
        SELECT gd.Name as var_rank, g.name as var_name
        FROM geography g
    ...
    higher_geography_loop: LOOP
        ...
        CASE var_geography_rank
            WHEN 'continent' THEN SET var_continent = var_geography_name;
            WHEN 'country' THEN SET var_country = var_geography_name;
            WHEN 'state' THEN SET var_state = var_geography_name;
            WHEN 'county' THEN SET var_county = var_geography_name;
            ELSE BEGIN END;
        END CASE;
    ...
    REPLACE INTO aux_highergeography_test (GeographyID, geographyRank,
        continent, country, `state`, county, higherGeography)
    VALUES (in_geography_id, in_geography_rank, 
        var_continent, var_country, var_state, var_county, var_higher_geography);
