...
FOR EACH ROW
  BEGIN
    ...
    IF isnull(@DISABLE_TRIGGER) THEN
      ...
      IF (NEW.Latitude1 IS NOT NULL AND NEW.Longitude1 IS NOT NULL) AND (NEW.Latitude1!=OLD.Latitude1 OR NEW.Longitude1!=OLD.Longitude1) AND var_who IN (1, 2) THEN
            
            IF var_who=2 THEN
              SET var_georeferencedby=NEW.ModifiedByAgentID;
